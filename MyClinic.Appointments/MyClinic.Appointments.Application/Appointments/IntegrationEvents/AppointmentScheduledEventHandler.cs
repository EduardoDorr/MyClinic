...
public sealed class AppointmentScheduledEventHandler : IDomainEventHandler<AppointmentScheduledEvent>
{
    ...
    public AppointmentScheduledEventHandler(IMessageBusProducerService messageBusService)
    {
    ...
    public Task Handle(AppointmentScheduledEvent notification, CancellationToken cancellationToken)
    {
        var sendEmailEvent =
            new SendEmailEvent(
                nameof(AppointmentScheduledEvent),
        ...
        _messageBusService.Publish(nameof(AppointmentScheduledEvent), notification);

        return Task.CompletedTask;
    }

    private static string CreateEmailMessage(AppointmentScheduledEvent notification)
    {
        var message = $@"
            <!DOCTYPE html>
            <html lang=""pt-br"">
