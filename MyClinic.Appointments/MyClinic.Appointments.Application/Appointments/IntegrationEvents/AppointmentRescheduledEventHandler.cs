...
public sealed class AppointmentRescheduledEventHandler : IDomainEventHandler<AppointmentRescheduledEvent>
{
    ...
    public Task Handle(AppointmentRescheduledEvent notification, CancellationToken cancellationToken)
    {
        ...
        _messageBusService.Publish(nameof(AppointmentRescheduledEvent), notification);

        return Task.CompletedTask;
    }

    private static string CreateEmailMessage(AppointmentRescheduledEvent notification)
    {
        var message = $@"
            <!DOCTYPE html>
            <html lang=""pt-br"">
