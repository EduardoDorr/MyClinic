...
public sealed class AddSchedulesCommandHandler : IRequestHandler<AddSchedulesCommand, Result>
{
    ...
    public async Task<Result> Handle(AddSchedulesCommand request, CancellationToken cancellationToken)
    {
        ...
        var uniquenessResult = await CheckSchedulesForUniquenessAsync(request.Schedules, request.DoctorId, cancellationToken);
    ...
    private async Task<Result> CheckSchedulesForUniquenessAsync(IList<DoctorSchedule> doctorSchedules, Guid doctorId, CancellationToken cancellationToken)
    {
        ...
        {
            var isUnique = await _scheduleRepository.IsUniqueAsync(doctorSchedule.StartDate, doctorSchedule.EndDate, doctorId, cancellationToken);
